...
import { ANALYTICS_RULES, AnalyticsRuleName } from '@/lib/services/typesense.service';
import type {
    CreateCollection,
    DeleteCollection,
    GetCollectionInfo,
...
export const createCollection: AppRouteHandler<CreateCollection> = async (c) => {
    ...
    try {
        ...
        throw new ApiError(`Failed to create ${name}: ${error instanceof Error ? error.message : 'Unknown error'}`, HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
};
...
export const getCollectionInfo: AppRouteHandler<GetCollectionInfo> = async (c) => {
    ...
    try {
        ...
        const collections = query?.name ? [query.name] : Object.keys(typesenseService.collections) as (keyof typeof typesenseService.collections)[];
