import { QuestionDocument, questionSchema } from "@/lib/typesense/schema";
import { Client } from "../typesense.service";
import { multisearch, multisearchEntry } from "typesense-ts";
import { GeneralQuestionParams } from "@/lib/typesense/schema/question.schema";
...
export class QuestionCollectionService extends CollectionService<QuestionDocument> {
    ...
    static readonly FILTER_FIELDS = {
        isSolved: [true, false] as const,
        authorId: [] as const // Can add author IDs dynamically if needed
    } as const;
    ...
    async searchGeneral(params: GeneralQuestionParams) {
        try {
            ...
            if (params.authorId) {
                ...
                filterString += `authorId:=${params.authorId}`;
            }
